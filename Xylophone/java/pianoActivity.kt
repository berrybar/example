...
class pianoActivity : AppCompatActivity() {
    ...
    private val soundPool = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
        //내 버전이 롤리팝과 같거나 높을 때
        //건반이 총 13개여서 13개로 초기화
        SoundPool.Builder().setMaxStreams(13).build()
    } else {
        //롤리팝보다 버전이 낮을 때 초기화 하는 법
        SoundPool(13, AudioManager.STREAM_MUSIC, 0)
    }

    //건반마다 필요한 소리를 연결해주는 list
    private val sounds = listOf(
        Pair(R.id.do1, R.raw.piano_do1),
        Pair(R.id.do2, R.raw.piano_do2),
        Pair(R.id.re1, R.raw.piano_re1),
        Pair(R.id.re2, R.raw.piano_re2),
        Pair(R.id.mi, R.raw.piano_mi),
        Pair(R.id.fa1, R.raw.piano_fa1),
        Pair(R.id.fa2, R.raw.piano_fa2),
        Pair(R.id.sol1, R.raw.piano_sol1),
        Pair(R.id.sol2, R.raw.piano_sol2),
        Pair(R.id.la1, R.raw.piano_la1),
        Pair(R.id.la2, R.raw.piano_la2),
        Pair(R.id.si, R.raw.piano_si),
        Pair(R.id.do3, R.raw.piano_do3)
    )
    ...
    override fun onDestroy() {
        super.onDestroy()
        //soundpool 해제 사용했으면 무조건 해제해줘야 함
        soundPool.release()
    }
