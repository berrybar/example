...
/*
    private fun movePosition(lat: Double, lng: Double) {
        val latLng = LatLng(lat, lng)
        mMap.animateCamera(CameraUpdateFactory.newLatLngZoom(latLng, 17f))
        mMap.addMarker(MarkerOptions().position(latLng).title("Marker in MyPosition"))
    }*/
    ...
    inner class MyLocationCallBack : LocationCallback() {
        override fun onLocationResult(locationResult: LocationResult?) {
            ...
            location?.run {
                ...
                if( move ) {
                    latLng = LatLng(latText.text.toString().toDouble() + ySpeed, lngText.text.toString().toDouble() + xSpeed)
                } else {
                    latLng = LatLng(latitude + ySpeed, longitude + xSpeed)
                }
